...
\end{tikzcd}\]
Note that \(\la \mathbb{L}^*,\mathbb{M}^*\ra=\mathrm{M}^*\mathbb{L}^*\) since the group \(\mathbb{L}^*\) is normal by Galois correspondence. By the second isomorphism theorems in groups, we know that \(\mathbb{L}^*\cap \mathbb{M}^*\) is normal in \(\mathbb{M}^*\) and we have an isomorphism 
